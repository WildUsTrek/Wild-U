...
(function () {
  ...
  var WILDU_MEDIA_READER_PAYLOAD_SCHEMA_VERSION = 1;
  ...
  function safeReaderPayloadPathPart(value) {
    return String(value || '')
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9_-]+/g, '-')
  ...
  function buildReaderPayloadObjectKey(mediaId, readerVersion) {
    return [
      'wildu-reader',
      'pdf',
      safeReaderPayloadPathPart(mediaId),
      'v' + Math.max(1, Number(readerVersion || 1)),
      'reader.json'
    ].join('/');
  }

  function sanitizeReaderBlocksForPublicPayload(blocks) {
    if (!Array.isArray(blocks)) return [];

    return blocks.map(function (block) {
      if (!block || typeof block !== 'object') return null;

      var type = String(block.type || 'paragraph').trim().toLowerCase();
      var role = String(block.role || '').trim().toLowerCase();

      if (type === 'html') return null;

      if (type === 'image' || type === 'img') {
        var url = String(block.url || block.src || '').trim();
        if (!url) return null;
        ...
        return {
          ...
          role: role,
          url: url,
          alt: String(block.alt || block.title || '').trim(),
          caption: String(block.caption || block.didascalia || '').trim(),
      ...
      if (type === 'divider' || type === 'hr') {
        return { type: 'divider', role: role };
      }

      var text = String(block.text || block.content || block.value || '').trim();
      if (!text) return null;

      if (['heading', 'h1', 'h2', 'title'].indexOf(type) >= 0) type = 'heading';
      else if (['lead', 'intro', 'summary'].indexOf(type) >= 0) type = 'lead';
      else if (['quote', 'blockquote'].indexOf(type) >= 0) type = 'quote';
      else if (['note', 'callout'].indexOf(type) >= 0) type = 'note';
      else type = 'paragraph';

      return { type: type, role: role, text: text };
    }).filter(Boolean);
  }

  function buildReaderPayloadJson(mediaId, heavy) {
    heavy = heavy || {};
    var publicBlocks = sanitizeReaderBlocksForPublicPayload(heavy.readerBlocks);

    return {
      schemaVersion: WILDU_MEDIA_READER_PAYLOAD_SCHEMA_VERSION,
      storageMode: 'r2-json-v1',
      mediaId: mediaId,
      mediaTitle: heavy.mediaTitle || '',
      readerVersion: heavy.readerVersion || 1,
      readerSourceMediaVersion: heavy.readerSourceMediaVersion || 1,
      readerStatus: heavy.readerStatus || 'READY',
      readerBlocks: publicBlocks,
      readerBlockCount: publicBlocks.length,
      readerCharCount: publicBlocks.reduce(function (sum, block) {
        return sum + String(block.text || '').length;
      }, 0),
      readerImageCount: publicBlocks.filter(function (block) { return block && block.type === 'image'; }).length,
      readerPreview: heavy.readerPreview || '',
      readerGeneratedAtClient: new Date().toISOString()
    };
  }
  ...
  function inferReaderPayloadObjectKeyFromUrl(publicUrl) {
    ...
    try {
      var url = new URL(publicUrl, window.location.href);
      var key = decodeURIComponent(String(url.pathname || '').replace(/^\/+/, ''));
      return /^wildu-reader\/pdf\//i.test(key) ? key : '';
    } catch (err) {
      return '';
    }
  }

  async function verifyReaderPayloadUrl(publicUrl) {
    publicUrl = String(publicUrl || '').trim();
    if (!publicUrl) throw new Error('URL pubblico payload reader mancante.');
    ...
    if (!res.ok) {
      throw new Error('Payload reader JSON non leggibile via URL pubblico HTTP ' + res.status + '.');
    }

    var payload = await res.json();
    var blocks = Array.isArray(payload && payload.readerBlocks)
      ? payload.readerBlocks
      : (Array.isArray(payload && payload.blocks) ? payload.blocks : []);

    if (!blocks.length) {
      throw new Error('Payload reader JSON leggibile ma senza readerBlocks.');
    }
  ...
  async function deleteReaderPayloadQuietly(item, reason) {
    item = item || {};
    var objectKey = String(item.readerPayloadObjectKey || item.reader_payload_object_key || '').trim();
    if (!objectKey) {
      objectKey = inferReaderPayloadObjectKeyFromUrl(
        item.readerPayloadUrl ||
        item.readerJsonUrl ||
        item.readerPayloadLocation ||
        item.reader_payload_url ||
        ''
      );
    }

    if (!objectKey || !/^wildu-reader\/pdf\//i.test(objectKey)) return;
    if (!root.R2WorkerService || typeof root.R2WorkerService.deleteObject !== 'function') return;

    try {
      await root.R2WorkerService.deleteObject(objectKey);
      console.log('[WILDU READER] Payload JSON cancellato:', { objectKey: objectKey, reason: reason || '' });
    } catch (err) {
      console.warn('[WILDU READER] Payload JSON non cancellato:', err);
    }
  }
  ...
  function getReaderPayloadUrlFromItem(item) {
    item = item || {};

    var direct = String(item.readerPayloadUrl || item.reader_json_url || item.readerJsonUrl || '').trim();
    if (direct) return direct;

    var location = String(item.readerPayloadLocation || item.reader_payload_location || '').trim();
    if (/^https?:\/\//i.test(location)) return location;
    if (/^r2:https?:\/\//i.test(location)) return location.replace(/^r2:/i, '');

    return '';
  }

  function readerIsR2Json(item) {
    item = item || {};

    return !!(
      getReaderPayloadUrlFromItem(item) &&
      String(item.readerStorageMode || '').toLowerCase() === 'r2-json-v1'
    );
  }

  function readerIsLegacyFirestore(item) {
    item = item || {};

    var location = String(item.readerPayloadLocation || '').trim();
    var storageMode = String(item.readerStorageMode || '').trim().toLowerCase();
    var hasDoc = !!String(item.readerDocPath || '').trim();

    return !!(
      hasReaderBuild(item) &&
      !readerIsR2Json(item) &&
      (
        /^firestore:/i.test(location) ||
  ...
  function readerIsStale(item) {
    item = item || {};

    var sourceVersion = Number(item.readerSourceMediaVersion || 0);
    var mediaVersion = Number(item.mediaVersion || 0);

    return !!(
      hasReaderBuild(item) &&
      sourceVersion > 0 &&
      mediaVersion > 0 &&
      sourceVersion < mediaVersion
    );
  }

  function readerDeliveryStatus(item) {
    if (!isPdfMediaForReader(item)) {
      return { kind: 'none', tone: '', label: '' };
    }

    if (isBookMediaItem(item)) {
      return hasReaderBuild(item)
        ? { kind: 'book-reader', tone: 'warn', label: 'libro: reader ottimizzato da pulire' }
        : { kind: 'book-fallback', tone: '', label: 'libro: lettura PDF fallback' };
    }

    if (!hasReaderBuild(item)) {
      return { kind: 'missing', tone: 'warn', label: 'reader: da generare' };
    }

    if (readerIsStale(item)) {
      return { kind: 'stale', tone: 'warn', label: 'reader obsoleto: rigenera JSON' };
    }
    ...
    var base = 'reader v' + version + (count ? ' - ' + count + ' blocchi' : '') + (imgCount ? ' - ' + imgCount + ' immagini' : '') + qualityLabel;

    if (readerIsR2Json(item)) {
      return {
        kind: 'r2-json',
        tone: quality !== null && quality < 60 ? 'warn' : 'good',
        label: 'R2 JSON - ' + base
      };
    }

    if (readerIsLegacyFirestore(item)) {
      return {
        kind: 'legacy-firestore',
        tone: 'warn',
        label: 'legacy Firestore: migra a Cloudflare'
      };
    }
    ...
    return {
      kind: 'unverified',
      tone: 'warn',
      label: 'reader JSON non verificato'
    };
  }
  ...
  function readerBuildChipHtml(item) {
    ...
    var status = readerDeliveryStatus(item);
    return '<span class="chip' + (status.tone ? ' ' + status.tone : '') + '">' + root.escapeHtml(status.label) + '</span>';
  }
  ...
  function readerBuildActionsHtml(item) {
    ...
    var buildLabel = status.kind === 'stale'
      ? 'Rigenera reader JSON'
      : (status.kind === 'missing' ? 'Genera reader JSON' : 'Genera reader + anteprima');

    return '' +
      (status.kind === 'legacy-firestore' || status.kind === 'unverified'
        ? '<button class="small good" data-migrate-pdf-reader="' + id + '">' + (status.kind === 'unverified' ? 'Verifica JSON Cloudflare' : 'Migra a JSON Cloudflare') + '</button>'
        : '') +
      '<button class="small" data-build-pdf-reader="' + id + '">' + buildLabel + '</button>' +
      (hasReaderBuild(item)
...
root.$('#module-description').value = item.description || item.Descrizione || item.notes || '';
  ...
  function splitReaderPatchForStorage(item, patch) {
    ...
    patch = root.PdfReaderService && typeof root.PdfReaderService.compactPatchForFirestore === 'function'
      ? root.PdfReaderService.compactPatchForFirestore(payloadPatch)
      : payloadPatch;
    ...
    return {
      ...
      light: light,
      heavy: heavy,
      payloadSource: Object.assign({}, payloadPatch, heavy)
    };
  }
  ...
  async function saveSplitReaderForMedia(item, patch) {
    ...
    try {
      var payloadUpload = await uploadReaderPayloadToR2(split.mediaId, split.payloadSource);
      var payloadVerification = null;

      try {
        payloadVerification = await verifyReaderPayloadUrl(payloadUpload && payloadUpload.publicUrl);
      } catch (verifyErr) {
        await deleteReaderPayloadQuietly({
          readerPayloadObjectKey: payloadUpload && payloadUpload.objectKey
        }, 'READER_PAYLOAD_VERIFY_FAILED');
        throw verifyErr;
      }

      if (payloadUpload && payloadUpload.publicUrl) {
        split.light.readerStorageMode = 'r2-json-v1';
        split.light.readerPayloadLocation = 'r2:' + payloadUpload.publicUrl;
        split.light.readerPayloadUrl = payloadUpload.publicUrl;
        split.light.readerPayloadObjectKey = payloadUpload.objectKey || null;
        split.light.readerPayloadContentType = 'application/json';
        split.light.readerPayloadSizeBytes = Number(payloadUpload.sizeBytes || 0);
        split.light.readerPayloadSchemaVersion = WILDU_MEDIA_READER_PAYLOAD_SCHEMA_VERSION;
        split.light.readerPayloadVerifiedAtClient = new Date().toISOString();
        split.light.readerPayloadVerifiedBlockCount = Number(payloadVerification && payloadVerification.blockCount || 0);

        split.heavy.readerStorageMode = 'r2-json-v1';
        split.heavy.readerPayloadLocation = 'r2:' + payloadUpload.publicUrl;
        split.heavy.readerPayloadUrl = payloadUpload.publicUrl;
        split.heavy.readerPayloadObjectKey = payloadUpload.objectKey || null;
        split.heavy.readerPayloadVerifiedAtClient = split.light.readerPayloadVerifiedAtClient;
        split.heavy.readerPayloadVerifiedBlockCount = split.light.readerPayloadVerifiedBlockCount;
      }
    } catch (payloadErr) {
      console.warn('[WILDU READER] Upload/verifica payload JSON R2 fallito: salvo solo Firestore split legacy.', payloadErr);
      split.light.readerPayloadLocation = 'firestore:' + split.docPath;
      split.light.readerStorageMode = 'split-v1';
    }
    ...
    try {
      ...
      try {
        await deleteReaderPayloadQuietly({
          readerPayloadObjectKey: split.light.readerPayloadObjectKey
        }, 'FIRESTORE_READER_SET_FAILED');
      } catch (_) {}
      throw err;
    }
  ...
  async function deleteSplitReaderForMedia(item) {
    ...
    await deleteReaderPayloadQuietly(item, 'DELETE_SPLIT_READER');
  ...
  function buildReaderClearCatalogPatch(item) {
    ...
    patch.readerPayloadUrl = root.FieldValue.delete();
    patch.readerPayloadObjectKey = root.FieldValue.delete();
    patch.readerPayloadContentType = root.FieldValue.delete();
    patch.readerPayloadSizeBytes = root.FieldValue.delete();
    patch.readerPayloadSchemaVersion = root.FieldValue.delete();
  ...
  async function migratePdfReaderToR2Json(id) {
    ...
    if (!isPdfMediaForReader(item)) {
      throw new Error('Migrazione reader disponibile solo per PDF.');
    }

    if (!canBuildPdfReaderForMedia(item)) {
      throw new Error('I Libri restano su lettura PDF fallback: non migro reader editoriali per questa categoria.');
    }

    if (readerIsR2Json(item) && !readerIsStale(item)) {
      var existingUrl = getReaderPayloadUrlFromItem(item);
      var verification = await verifyReaderPayloadUrl(existingUrl);
      root.toast('Reader R2 JSON gia verificato: ' + Number(verification.blockCount || 0) + ' blocchi.', 'success');
      return;
    }

    if (readerIsStale(item)) {
      throw new Error('Questo reader e obsoleto rispetto alla versione media. Usa "Rigenera reader JSON" per ricostruirlo dal PDF aggiornato.');
    }

    var existingPayloadUrl = getReaderPayloadUrlFromItem(item);
    if (existingPayloadUrl && !readerIsLegacyFirestore(item)) {
      var existingVerification = await verifyReaderPayloadUrl(existingPayloadUrl);
      var existingUser = root.requireCurrentUser();
      var existingPatch = {
        readerStorageMode: 'r2-json-v1',
        readerPayloadLocation: 'r2:' + existingPayloadUrl,
        readerPayloadUrl: existingPayloadUrl,
        readerPayloadContentType: 'application/json',
        readerPayloadSchemaVersion: WILDU_MEDIA_READER_PAYLOAD_SCHEMA_VERSION,
      ...
      await root.MediaService.updateMedia(item.id, existingPatch);
      root.toast('Reader JSON Cloudflare verificato e metadata riallineata: ' + Number(existingVerification.blockCount || 0) + ' blocchi.', 'success');
      await refreshTags();
      await refreshMedia();
      return;
    }
    ...
    var readerDoc = await loadReaderDocForMedia(item);
    var source = Object.assign({}, item, readerDoc || {});
    var blocks = Array.isArray(source.readerBlocks) ? source.readerBlocks : [];
    ...
    try {
      ...
      var patch = {
        ...
        readerPayloadObjectKey: payloadUpload.objectKey || null,
        readerPayloadContentType: 'application/json',
        readerPayloadSizeBytes: Number(payloadUpload.sizeBytes || 0),
        readerPayloadSchemaVersion: WILDU_MEDIA_READER_PAYLOAD_SCHEMA_VERSION,
      ...
      try {
        await getReaderDocRef(item.id).set({
          ...
          readerPayloadObjectKey: patch.readerPayloadObjectKey,
          readerPayloadContentType: patch.readerPayloadContentType,
          readerPayloadSizeBytes: patch.readerPayloadSizeBytes,
          readerPayloadSchemaVersion: patch.readerPayloadSchemaVersion,
          readerPayloadVerifiedAtClient: patch.readerPayloadVerifiedAtClient,
          readerPayloadVerifiedBlockCount: patch.readerPayloadVerifiedBlockCount
        }, { merge: true });
      } catch (legacyErr) {
        console.warn('[WILDU READER] Catalogo aggiornato, ma documento legacy non riallineato:', legacyErr);
      }
      ...
      if (payloadUpload && payloadUpload.objectKey) {
        await deleteReaderPayloadQuietly({
          readerPayloadObjectKey: payloadUpload.objectKey
        }, 'READER_MIGRATION_FAILED');
      }
      throw err;
    }
  }
  ...
  async function buildPdfReaderForMedia(id) {
    ...
    var previousReaderDoc = null;
    try {
      previousReaderDoc = await loadReaderDocForMedia(item);
    } catch (oldReaderErr) {
      console.warn('[WILDU READER] Vecchio reader non letto prima della sostituzione:', oldReaderErr);
    }

    var previousReaderForCleanup = previousReaderDoc
      ? Object.assign({}, item, previousReaderDoc)
      : item;

    var savedReader = null;
    try {
      savedReader = await saveSplitReaderForMedia(item, patch);
    } catch (saveErr) {
      await cleanupPdfReaderImagesQuietly(patch, 'READER_SAVE_FAILED_NEW_IMAGES');
      throw saveErr;
    }

    // Pulisce solo dopo commit del nuovo reader: se il salvataggio fallisce, il vecchio resta integro.
    await cleanupPdfReaderImagesQuietly(previousReaderForCleanup, 'READER_REPLACE_OLD_IMAGES');
    await deleteReaderPayloadQuietly(previousReaderForCleanup, 'READER_REPLACE_OLD_PAYLOAD');

    var savedMode = savedReader &&
      savedReader.light &&
      savedReader.light.readerStorageMode === 'r2-json-v1'
        ? 'R2 JSON verificato + archivio Firestore admin'
        : 'split Firestore legacy';

    root.toast(
      'Reader editoriale V6.2 salvato in modalita ' + savedMode + ': ' +
